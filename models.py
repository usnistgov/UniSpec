...
#         self.Wq = nn.Parameter(initQ((in_ch, dk, h)), requires_grad=True)
#         self.Wk = nn.Parameter(initK((in_ch, dk, h)), requires_grad=True)
#         self.Wv = nn.Parameter(initV((in_ch, dv, h)), requires_grad=True)
        
#         self.Wo = nn.Parameter(initO((self.dv*self.h, self.out_dim)), requires_grad=True)
#         self.drop = nn.Identity() if drop==0 else nn.Dropout(drop)
        
#     def forward(self, inp, mask):
#         Q = torch.relu(torch.einsum('abc,bde->adce', inp, self.Wq))
#         K = torch.relu(torch.einsum('abc,bde->adce', inp, self.Wk))
#         V = torch.relu(torch.einsum('abc,bde->adce', inp, self.Wv))
        
#         QK = torch.einsum('abcd,abed->aced', Q, K) / Q.shape[1]**0.5
#         FM = torch.softmax(QK, dim=2) - mask[:, None, :, None]
#         FMV = torch.einsum('abcd,aecd->abed', FM, V)
        
#         FMV = FMV.reshape(FMV.shape[0], -1, FMV.shape[2]*FMV.shape[-1])
#         resid = torch.einsum('abc,cd->adb', FMV, self.Wo)
        
#         return inp + self.drop(resid), (), FM
